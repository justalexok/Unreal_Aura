...
UOverlayWidgetController* UAuraAbilitySystemLibrary::GetOverlayWidgetController(const UObject* WorldContextObject)
{
	...
	AAuraHUD* AuraHUD;
	if (MakeWidgetControllerParams(WorldContextObject, WCParams, AuraHUD)) //If we successfully make the params
	{
...
UAbillityInfo* UAuraAbilitySystemLibrary::GetAbilityInfo(const UObject* WorldContextObject)
{
	const AAuraGameModeBase* AuraGameMode = Cast<AAuraGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject));
	if (AuraGameMode == nullptr ) return nullptr;
...
bool UAuraAbilitySystemLibrary::IsSuccessfulDebuff(FGameplayEffectContextHandle& EffectContextHandle)
{
...
float UAuraAbilitySystemLibrary::GetDebuffDamage(FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
...
float UAuraAbilitySystemLibrary::GetDebuffDuration(FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
...
float UAuraAbilitySystemLibrary::GetDebuffFrequency(FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
	...
	const FVector& SphereOrigin)
{
	...
	if (const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			//If actor implements interface and is alive
	...
	ECharacterClass CharacterClass, int32 CharacterLevel)
{
	...
	const FScalableFloat XPRewardScalable = CharacterClassInfo->GetClassDefaultInfo(CharacterClass).XPReward;
	const float XPReward = XPRewardScalable.GetValueAtLevel(CharacterLevel);
