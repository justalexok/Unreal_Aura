						 ...
						 const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		const float TypeDamage = Spec.GetSetByCallerMagnitude(DamageType,false,-1); //Returns set by caller mag of DamageType, and -1 if doesn't find
		if (TypeDamage > -0.9f)  //0.1 padding for floating point imprecision
		{
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	//Get Damage Set by Caller Magnitude (See ProjectileSpell.cpp)
	float Damage = 0.f;
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		float DamageTypeValue = Spec.GetSetByCallerMagnitude(Pair.Key, false); //get the "set by caller magnitude" for each damage type. If not set in blue print will be null, but we won't be warned in log
	...
	//Get Coeffecient value from curve table
	...
	float EffectiveCriticalHitChance = CriticalHitChance - TargetCriticalHitResistance * CriticalHitResistanceCoefficient;
